...
all_fileids_cmu = []
all_transcriptions_cmu = []
all_fileids_hrt = []
all_transcriptions_hrt = []
all_fileids_nn = []
all_transcriptions_nn = []
...
def find_files(my_home, duration_file, wav_files, txt_files, dbname):
    ...
    for content in ok_names:
        ...
        if my_home.count("Battelli") > 0:
            ...
            if my_home.count("16") > 0:
                all_fileids_cmu.append(content) 
            else:
                if my_home.count("BCN") > 0:
                    all_fileids_hrt.append(content) 
                else:
                    if my_home.count("veprad") > 0:
                        all_fileids_nn.append(content)
                    else:
    ...
    file_names = open(my_home + "/db_files/my_db13_" + dbname + ".fieldids", "w")
    ...
    for new_content in ok_text: 
        ...
        if my_home.count("Battelli") > 0:
            ...
            if my_home.count("16") > 0:
                all_transcriptions_cmu.append(new_content)
            else:
                if my_home.count("BCN") > 0:
                    all_transcriptions_hrt.append(new_content) 
                else:
                    if my_home.count("veprad") > 0:
                        all_transcriptions_nn.append(new_content)
                    else:
                        all_transcriptions.append(new_content)

    file_text = open(my_home + "/db_files/my_db13_" + dbname + ".transcription", "w") 
...
for new_dir_cmu in new_cmu_dir:
    ...
    if new_dir_cmu.count("txt") > 0:
        continue

    if new_dir_cmu.count("tgz") > 0:
        continue

    print("Sample CMU " + new_dir_cmu)  

    lines_all, messed_up = find_files(basedir + cmu_dir, new_dir_cmu + ".txt", new_dir_cmu  + "/wav", "Prompts/txt/" + new_dir_cmu, new_dir_cmu)
    
    for line in lines_all:
        lines_cmu.append(line)         

    for line in messed_up:
        messed_up_cmu.add(line)

    print("Broj zapisa uzorka", new_dir_cmu + ":", len(lines_all), "Sekunde:", sum(lines_all), "Minute:", sum(lines_all) / 60, "Sati:", sum(lines_all) / 3600, "Maksimum:", max(lines_all))
...
print("HRT uzorak") 
lines_all, messed_up = find_files(basedir + "veprad/podaci-BCN/", "trajanja_hrt_novo.txt", "audio_kopija", "tekst_popravljeno", "hrt")
print("Broj zapisa HRT uzorka:", len(lines_all), "Sekunde:", sum(lines_all), "Minute:", sum(lines_all) / 60, "Sati:", sum(lines_all) / 3600, "Maksimum:", max(lines_all))
'''
print("Sinteza uzorak sz04") 
lines_all, messed_up = find_files(basedir + "veprad/podaci_sinteza/sz04/", "audio/trajanje_sz04.txt", "audio/wav", "text", "sz04")
print("Broj zapisa sinteza uzorka sz04:", len(lines_all), "Sekunde:", sum(lines_all), "Minute:", sum(lines_all) / 60, "Sati:", sum(lines_all) / 3600, "Maksimum:", max(lines_all))
'''
print("Sinteza uzorak sz08")
lines_all, messed_up = find_files(basedir + "veprad/podaci_sinteza/sz08/", "audio/trajanje_sz08.txt", "audio/wav", "text", "sz08")
print("Broj zapisa sinteza uzorka sz08:", len(lines_all), "Sekunde:", sum(lines_all), "Minute:", sum(lines_all) / 60, "Sati:", sum(lines_all) / 3600, "Maksimum:", max(lines_all))

print("Radio uzorak")
lines_all, messed_up = find_files(basedir + "veprad/podaci_radio/", "audio/trajanje_prvi.txt", "audio/prvi", "text/prvi", "prvi")
print("Broj zapisa radio uzorka:", len(lines_all), "Sekunde:", sum(lines_all), "Minute:", sum(lines_all) / 60, "Sati:", sum(lines_all) / 3600, "Maksimum:", max(lines_all))
...
print("Broj zapisa uzoraka CMU:", len(lines_cmu), "Sekunde:", sum(lines_cmu), "Minute:", sum(lines_cmu) / 60, "Sati:", sum(lines_cmu) / 3600, "Maksimum:", max(lines_cmu), "Pogreška:", len(messed_up_cmu), messed_up_cmu)

phone_unknown_file = open(basedir + "/my_db13.filler", "w") 
...
dict_file = open(basedir + "/my_db13.dic", "w") 
...
phone_known_file = open(basedir + "/my_db13.phone", "w") 
phone_known_file.write((text_phones + "SIL\n" + "NOISE"))
phone_known_file.close()
 
all_transcriptions_file = open(basedir + "/my_db13.transcription", "w") 
...
all_fileids_file = open(basedir + "/my_db13.fileids", "w") 
...
all_fileids_cmu_train, all_fileids_cmu_test, all_transcriptions_cmu_train, all_transcriptions_cmu_test = train_test_split(all_fileids_cmu, all_transcriptions_cmu, test_size=0.1, random_state=42)
all_fileids_nn_train, all_fileids_nn_test, all_transcriptions_nn_train, all_transcriptions_nn_test = train_test_split(all_fileids_nn, all_transcriptions_nn, test_size=0.1, random_state=42)
all_fileids_hrt_train, all_fileids_hrt_test, all_transcriptions_hrt_train, all_transcriptions_hrt_test = train_test_split(all_fileids_hrt, all_transcriptions_hrt, test_size=0.1, random_state=42)

all_transcriptions_test_file = open(basedir + "/my_db13_test.transcription", "w") 
...
for transcription_test in all_transcriptions_cmu_test:
    text_transcriptions_test += transcription_test + "\n"

for transcription_test in all_transcriptions_hrt_test:
    text_transcriptions_test += transcription_test + "\n"

for transcription_test in all_transcriptions_nn_test:
    text_transcriptions_test += transcription_test + "\n"

all_transcriptions_test_file.write(text_transcriptions_test)
all_transcriptions_test_file.close()
 
all_fileids_test_file = open(basedir + "/my_db13_test.fileids", "w") 
...
for fileid_test in all_fileids_cmu_test:
    text_fileids_test += fileid_test + "\n"

for fileid_test in all_fileids_hrt_test:
    text_fileids_test += fileid_test + "\n"

for fileid_test in all_fileids_nn_test:
    text_fileids_test += fileid_test + "\n"

all_fileids_test_file.write(text_fileids_test)
all_fileids_test_file.close()

all_transcriptions_train_file = open(basedir + "/my_db13_train.transcription", "w") 

text_transcriptions_train = ""

for transcription_train in all_transcriptions:
    text_transcriptions_train += transcription_train + "\n"

for transcription_train in all_transcriptions_new_train:
    text_transcriptions_train += transcription_train + "\n"

for transcription_train in all_transcriptions_cmu_train:
    text_transcriptions_train += transcription_train + "\n"

for transcription_train in all_transcriptions_hrt_train:
    text_transcriptions_train += transcription_train + "\n"

for transcription_train in all_transcriptions_nn_train:
    text_transcriptions_train += transcription_train + "\n"

all_transcriptions_train_file.write(text_transcriptions_train)
all_transcriptions_train_file.close()
 
all_fileids_train_file = open(basedir + "/my_db13_train.fileids", "w") 

text_fileids_train = ""

for fileid_train in all_fileids:
    text_fileids_train += fileid_train + "\n"

for fileid_train in all_fileids_new_train:
    text_fileids_train += fileid_train + "\n"

for fileid_train in all_fileids_cmu_train:
    text_fileids_train += fileid_train + "\n"

for fileid_train in all_fileids_hrt_train:
    text_fileids_train += fileid_train + "\n"

for fileid_train in all_fileids_nn_train:
    text_fileids_train += fileid_train + "\n"
