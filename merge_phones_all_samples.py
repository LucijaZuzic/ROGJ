...
for phone in all_phones:  
    ...
    for dirs_to_include in dirs: 
            filename_table_corr = basedir + "all_results/" + dirs_to_include + " " + phone + " Točni znakovi.csv" 
            ...
            filename_table_incorr = basedir + "all_results/" + dirs_to_include + " " + phone + " Netočni znakovi.csv" 
            ...
            filename_table_sub = basedir + "all_results/" + dirs_to_include + " " + phone + " Zamijenjeni znakovi.csv" 
            table_new_sub = open_table(filename_table_sub) 
            #print(table_new_sub) 
            tables_sub.append(table_new_sub) 
            filename_table_ins = basedir + "all_results/" + dirs_to_include + " " + phone + " Dodani znakovi.csv" 
            table_new_ins = open_table(filename_table_ins) 
            #print(table_new_ins) 
            tables_ins.append(table_new_ins) 
            filename_table_del = basedir + "all_results/" + dirs_to_include + " " + phone + " Ispušteni znakovi.csv" 
            table_new_del = open_table(filename_table_del) 
            #print(table_new_del) 
            tables_del.append(table_new_del) 
            
            phone_table = add_tables([duplicate_table(table_new_corr), duplicate_table(table_new_incorr)])
            res_table = string_array_to_csv(phone_table) 
            filename_table = basedir + "all_results/" + dirs_to_include + " " + phone + " Svi znakovi.csv" 
            print(res_table)
    ...
    phone_table = add_tables([duplicate_table(ok_tables), duplicate_table(notok_tables)])
    res_table = string_array_to_csv(phone_table) 
    filename_table = basedir + "all_results/All dirs " + phone + " Svi znakovi.csv" 
    print(res_table)
    #print(filename_table)
    file_open = open(filename_table, "w")
    file_open.write(res_table)
    ...
    names = ["Ispušteni", "Dodani", "Zamijenjeni", "Točni", "Netočni"]
    for num in range(len(names)):
        for table_ind in range(len(tables_percent[num][1:])):
            res_table = string_array_to_csv(tables_percent[num][table_ind + 1]) 
            filename_table = basedir + "all_results/All dirs " + phone + " " + names[num] + " znakovi postotak.csv" 
            #print(res_table)
            #print(filename_table)
            file_open = open(filename_table, "w")
            file_open.write(res_table)
        ...
        for table_ind in range(len(tables_number[num])):
            ...
            filename_table = basedir + "all_results/All dirs " + phone + " " + names[num] + " znakovi.csv" 
            #print(res_table)
            #print(filename_table)
            file_open = open(filename_table, "w")
            file_open.write(res_table)
